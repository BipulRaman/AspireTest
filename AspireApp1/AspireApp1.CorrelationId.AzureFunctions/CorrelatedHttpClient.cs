...
/// <summary>
/// Service for making HTTP calls with automatic correlation ID propagation in Azure Functions
/// </summary>
...
/// <summary>
/// Implementation of correlated HTTP client for Azure Functions
/// </summary>
public class CorrelatedHttpClient : ICorrelatedHttpClient
{
    ...
    {
        _logger.LogInformation("Azure Function making {Method} request to {Uri}", method, requestUri);
    }
    ...
    private void LogResponse(string method, string requestUri, HttpResponseMessage response)
    {
        if (response.IsSuccessStatusCode)
        {
            _logger.LogInformation("Azure Function {Method} request to {Uri} completed with {StatusCode}", 
                method, requestUri, (int)response.StatusCode);
        }
        else
        {
            _logger.LogWarning("Azure Function {Method} request to {Uri} failed with {StatusCode}", 
                method, requestUri, (int)response.StatusCode);
        }
    }
}
